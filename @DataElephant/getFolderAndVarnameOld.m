function [filename,cumhash,varname,ori_id] = getFolderAndVarnameOld(obj,hash,~)
    ...
    for ii=1:size(u_id_cum,1)
        ...
        if isempty(u_id_cum)
            filename{ii} = [obj.oldrootfolder '\' obj.steps(1).name '\D.mat'];
        else
            filename{ii} = [obj.oldrootfolder '\' obj.steps(1).name '\' hexhash(obj,u_id_cum(ii,(end-(obj.hashlength-1)):end)) '\D.mat'];
        end
    end
end