...
p.addParamValue('legend',0,@(x) x{1});
p.addParamValue('fontsize',6,@(x) isnumeric(x) );
...
% generate correct path to file depending on system
...
%       end
    ...
    if NewFontStyle
      ...
      fprintf(fid,['\n%%\n\\providecommand\\%s%s{\\color[rgb]{%.3f,%.3f,'...
        ...
        CurrentColour(3),p.Results.fontsize,p.Results.fontsize,Angle,Weight,Fixed);
      NewFontStyle = 0;
    end
      ...
      if ~isempty(ticklabels)
        ...
        if strcmpi(get(handle,[jj,'scale']),'log') && AutoTickLabel.(jj)
          ...
          if all(~isnan(str2double(ticklabelcell)))
            ...
            % 10$^{\textrm{1}}$
            ticklabels = cellfun(@(x) ['10$^{\textrm{',RemoveSpaces(x),'}}$'],...
        ...
        elseif strcmpi(get(handle,[jj,'scale']),'linear') && AutoTickLabel.(jj)
          ...
          if abs(scale-1) > 1e-3
            ...
            LatexScale = ['$\times$ 10$^{\textrm{',num2str(round(scale)),'}}$'];
...
% from any string.
  function cropped_string = RemoveSpaces(string)
    ...
    cropped_string = AddDollarSigns(cropped_string);
  end
  
  function dollar_string = AddDollarSigns(string)
    if iscell(string)
      string = string{:};
    end
    ...
    if ischar( string )
        ...
        if isempty(strfind(string,'$'))
            dollar_string = regexprep(string,'([\^][\{][\(\\\w0-9\-\s)\)]*[\}])|([\^][\w0-9\)\(]*)','\$$1\$');
        else
            dollar_string = string;
        end
    else
