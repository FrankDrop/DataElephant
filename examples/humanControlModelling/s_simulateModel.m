function step = s_simulateModel()
    ...
    step.optional   = {'K_t_y','K_t_phi','K_e_y','t_e_y','K_e_phi','K_y','K_phi','tau_t_y','tau_t_phi','tau_e_y','tau_y','tau_e_phi','tau_phi','K_p_y', ...
                       ...
                       'Ypt_1','Ypt_2','Ypt_3',...
                       'omega_n','zeta_n'};
    step.output     = {'t','t;u','t;u_prenms','t;u_p_t','t;u_p_e','t;y','t;e','t;ft_total','t;n','model_s','t;phi_v','t;phi_m','t;u_p_phi'};
    step.handle     = @myfunc;
    step.version    = 1;
    step.saveme     = 0;
    step.memorizeme = 1;
    ...
    function [r_n] = myfunc(z,r,verbose,~)
        ...
        %% Run the model
        [x_s,x_e,x_m] = model.runTaskModel(model.getRealP(),z);

        %% Save the results
        fn      = fieldnames(x_m);

        for oo=1:length(fn)
            if any(strcmp(fn{oo},cellfun(@(x)DataElephant.splitOutput(x),step.output,'UniformOutput',false)))
                r_n.(fn{oo})  = x_m.(fn{oo});
            end
        end
        ...
        if verbose
            ...
            figure
                for oo=1:length(fn)
                    plot(x_s.t,x_s.(fn{oo})); hold all
                end
            fn      = fieldnames(x_e);
            
            figure
                for oo=1:length(fn)
                    plot(x_e.t,x_e.(fn{oo})); hold all
                end
            figure(cf);
        end
